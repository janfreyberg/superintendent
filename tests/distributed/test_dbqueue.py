...
@given(inp=one_of(series(dtype=int), series(dtype=float), series(dtype=str)))
...
@given(
    inp=(nested_strategy | primitive_strategy),
...
def test_insert_and_list():
    ...
    assert len(ids) == len(x) == len(y) == 1
    assert y[0] == "test_label"
    assert x[0] in ("test data 1", "test data 2")
    assert ids[0] == id_

    # check the other task is listed as uncompleted
    ids, x = q.list_uncompleted()
    assert len(ids) == len(x) == 1
    assert x[0] in ("test data 1", "test data 2")
    assert ids[0] != id_
