...
def test_default_display(mocker):
    disp_mock = mocker.patch("IPython.display.display")
    default_display_func("test data")
    assert disp_mock.called_once_with("test data")


def test_image_display_no_shape(mocker):
    disp_mock = mocker.patch("matplotlib.axes.Axes.imshow")
    test_img = np.random.rand(10, 10)
    image_display_func(test_img)
    assert disp_mock.called_once_with(test_img)
...
def test_image_display_explicit_shape(mocker):
    ...
    assert disp_mock.called_once_with(test_img.reshape(20, 20))
